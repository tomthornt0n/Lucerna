...
{
 F32 aspect;
 stop_audio_source(global_current_level->level.music);
 
 // NOTE(tbt): draw background
 aspect = (F32)global_current_level->level.bg->texture.height / (F32)global_current_level->level.bg->texture.width;
 world_draw_sub_texture(rectangle_literal(0.0f, 0.0f,
                                          SCREEN_WIDTH_IN_WORLD_UNITS,
                                          SCREEN_WIDTH_IN_WORLD_UNITS * aspect),
                        WHITE,
                        global_current_level->level.bg,
                        ENTIRE_TEXTURE);
 ...
 // NOTE(tbt): edit entities
 {
            ...
            400.0f)
  {
   ...
   {
    Entity *e = allocate_and_push_entity(&global_current_level->level);
  ...
  Entity *prev = NULL;
  for (Entity *e = global_current_level->level.entities;
       NULL != e;
       prev = e, e = e->next)
  {
   if (e->flags & 1 << ENTITY_FLAG_marked_for_removal)
   {
    ...
    {
     global_current_level->level.entities = e->next;
    }
 ...
 aspect = (F32)global_current_level->level.fg->texture.height / (F32)global_current_level->level.fg->texture.width;
 world_draw_sub_texture(rectangle_literal(0.0f, 0.0f,
                                          SCREEN_WIDTH_IN_WORLD_UNITS,
                                          SCREEN_WIDTH_IN_WORLD_UNITS * aspect),
                        WHITE,
                        global_current_level->level.fg,
                        ENTIRE_TEXTURE);
